...
export function ApplicationsPage() {
  ...
  const myApps = mockApplications.filter((a) => a.userId === user.id);

  return (
    <div className="mx-auto max-w-[1200px] px-4 py-8 md:px-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">My Applications</h1>
        <p className="mt-1 text-muted-foreground">{myApps.length} application{myApps.length !== 1 ? "s" : ""} submitted</p>
      </div>
      ...
      {myApps.length === 0 ? (
        <EmptyState
          ...
          action={<Button asChild><Link to={PATHS.JOBS}>Browse Jobs</Link></Button>}
        />
      ) : (
